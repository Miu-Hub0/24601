-- AUTO FARM LEVEL + FAST ATTACK + BRING MOB - ĐẦY ĐỦ SEA 1,2,3
-- Tương thích Delta, Solara, Arceus X, CodeX
...
if not task then
    task = {
        wait = function(t) wait(t or 0) end,
...
if not loadSuccess then
    game:GetService("StarterGui"):SetCore("SendNotification", {
        Title = "Lỗi",
        Text = "Không thể tải Rayfield! Vui lòng thử lại.",
        Duration = 5
    })
    return
end
...
local function GetCurrentQuest()
    ...
    local best = QuestData[#QuestData]
...
print("⚙️ Settings: Fast Attack + Bring Mob + Auto Stats")